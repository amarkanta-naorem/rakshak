...
export default function EmployeeAttendance() {
  ...
  const filteredEmployees: Employee[] = allEmployees.filter((emp) => {
    const matchesNameOrId = emp.name.toLowerCase().includes(inputValue.toLowerCase()) || emp.id.toLowerCase().includes(inputValue.toLowerCase());
    const matchesRole = selectedRole ? emp.userRole.toLowerCase() === selectedRole.toLowerCase() : true;
    return matchesNameOrId && matchesRole;
  });
            ...
            {isBulkActionsActive ? (
              <div className="flex items-center justify-between space-x-2 mb-3">
                ...
                <div className="relative">
                  <div className="relative">
                    <input id="employee-name" className="peer w-full bg-transparent text-gray-700 text-sm border border-gray-300 rounded-md px-3 py-2 pr-10 transition duration-300 ease-in-out focus:outline-none focus:border-blue-500 hover:border-blue-200 focus:shadow" placeholder=" " value={isBulkActionsActive ? inputValue : ""} onChange={(e) => isBulkActionsActive && setInputValue(e.target.value)} onKeyDown={(e) => isBulkActionsActive && handleKeyDown(e)}/>
                    <label htmlFor="employee-name" className={`absolute pointer-events-none bg-gradient-to-br from-gray-50 to-gray-100 px-1 left-3 text-gray-400 text-sm transition-all duration-300 ${inputValue && isBulkActionsActive ? "-top-2 text-[13px] text-blue-600" : "top-2 peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-400"} peer-focus:-top-2 peer-focus:text-[13px] peer-focus:text-blue-600`}>Search</label>
                  </div>
                  ...
                  <div className="flex">
                    {getDateRangeDays().length > 0 ? (
                      getDateRangeDays().map((day, index) => (
                        <div key={index} className="flex flex-col items-center justify-center border-r border-b border-gray-200 w-[12rem] min-w-[12rem] h-[5.5rem] px-2 bg-white">
                          <span className="text-sm font-semibold text-gray-800">{format(day, "MMM d, yyyy")}</span>
                        </div>
                      ))
                    ) : (
                      <div className="flex flex-col items-center justify-center border-r border-b border-gray-200 w-[12rem] min-w-[12rem] h-[5.5rem] px-2 bg-white">
                        <span className="text-sm font-semibold text-gray-800">Select a date range</span>
                      </div>
                    )}
                  </div>
                </div>
                        ...
                        <div key={employee.id} className="flex">
                          {getDateRangeDays().length > 0 ? (
                            getDateRangeDays().map((day, dayIndex) => {
                              const attendance = filteredAttendance(employee).find((att) => isSameDay(parseISO(att.date), startOfDay(day)));
                              return (
                                <div key={dayIndex} className={`flex flex-col justify-center border-r border-b border-gray-300 w-[12rem] min-w-[12rem] h-[5.5rem] px-2 relative ${
                                  attendance?.status === "present" ? "bg-green-50" :
                                  attendance?.status === "absent" ? "bg-red-50" :
                                  attendance?.status === "late" ? "bg-yellow-50" :
                                  attendance?.status === "half_day" ? "bg-orange-50" : "bg-white"
                                }`}>
                                  {attendance?.status === "present" ? (
                                    <div className="flex flex-col items-center space-y-2">
                                      ...
                                      <div className="text-xs text-gray-600">
                                        {(() => {
                                          const hours = Math.floor(parseFloat(attendance.totalWorkingHour));
                                          const minutes = Math.round((parseFloat(attendance.totalWorkingHour) - hours) * 60);
                                          if (minutes === 0) {
                                            return `Time logged : ${hours} hrs`;
                                          } else {
                                            return `Time logged : ${hours}h ${minutes}m`;
                                          }
                                        })()}
                                      </div>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-center">
                                      <div className="text-xs font-medium text-center">{attendance.reason}</div>
                                    </div>
                                  ) : attendance ? (
                                    <div className="flex flex-col items-center space-y-2">
                                      <div className="absolute top-1 left-1 text-[9px] text-center bg-rose-600 text-white w-[3rem] rounded-lg shadow-lg font-medium">
                                        {attendance.status.replace(/_/g, ' ').toLowerCase().replace(/^\w/, c => c.toUpperCase())}
                                      </div>
                                      <div className="pt-5">
                                        <div className="grid grid-cols-3 text-gray-800 text-xs text-center mb-1">
                                          <div>In</div>
                              ...
                              return (
                                <td key={day ? day.toISOString() : `empty-${weekIndex}-${dayIndex}`} className={`border-b border-r border-gray-200 relative transition-all duration-300 hover:bg-blue-50 hover:shadow ${ attendance?.status === "present" ? "bg-green-50" : attendance?.status === "absent" ? "bg-red-50" : "bg-white" }`} style={{ width: "14.2857%" }}>
                                  <div className="relative h-full flex flex-col p-1">
                                    {day && (
                                      <span className="text-sm font-semibold flex items-end justify-end">
                                        <span className={`${ isToday ? "text-white bg-blue-800 rounded-full w-5 h-5" : "text-gray-800" }`}>{format(day, "d")}</span>
                                      </span>
                                    )}
                                    <div className="flex-grow pt-2 text-xs text-gray-700">
                                      {attendance?.status === "present" ? (
                                        <>

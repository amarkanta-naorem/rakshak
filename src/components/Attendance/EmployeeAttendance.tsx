...
export default function EmployeeAttendance() {
    ...
    const filteredRoles: string[] = roleOptions.filter((role) =>
                    ...
                    <div className="w-[20rem] h-[72vh] border border-gray-300 rounded-md flex flex-col bg-white">
                        <div className="flex items-center justify-between space-x-3 p-2 w-full">
                            <div className="relative w-2/3">
                                <input
                                    id="client-name"
                                    className="peer w-full bg-transparent text-gray-700 text-sm border border-gray-300 rounded-md px-3 py-2 pr-10 transition duration-300 ease-in-out focus:outline-none focus:border-blue-500 hover:border-blue-200 shadow-sm focus:shadow"
                                    placeholder=" "
                                    value={inputValue}
                                ...
                                <label
                                    htmlFor="client-name"
                                    className={`absolute pointer-events-none bg-white px-1 left-3 text-gray-400 text-sm transition-all duration-300 ${inputValue ? '-top-2 text-xs text-blue-600' : 'top-2.5 peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-400'} peer-focus:-top-2 peer-focus:text-xs peer-focus:text-blue-600`}
                                >
                                    Employee Name
                                </label>
                            </div>
                            <div className="w-1/3 relative">
                                <div
                                    className="bg-white border border-gray-300 text-[#797979] text-sm w-full shadow-sm cursor-pointer px-3 py-2 rounded-md"
                                ...
                                {isDropdownOpen && (
                                    <div className="absolute z-10 bg-white border border-gray-300 rounded-lg shadow mt-1 w-full">
                                        <input
                                            type="text"
                                            placeholder="Search..."
                                            className="w-full p-1.5 border-b border-gray-300 text-sm text-gray-500 outline-none"
                                            value={roleSearch}
                                            ...
                                            <div className="absolute right-0 mt-2 w-72 bg-white border border-gray-200 rounded-xl shadow-2xl z-20 p-4">
                                                ...
                                                <div className="grid grid-cols-3 gap-2 mb-4">
                                                    {months.map((month, index) => (
                                                        <button
                                                            key={index}
                                                            className={`text-sm py-2 px-3 rounded-md ${selectedMonth === index ? 'bg-blue-600 text-white shadow-sm' : 'bg-gray-100 text-gray-700 hover:bg-blue-100 hover:text-blue-700'}`}
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {years.map((year) => (
                                                        <button
                                                            key={year}
                                                            className={`text-sm py-1 px-4 rounded-md ${selectedYear === year ? 'bg-blue-600 text-white shadow-sm' : 'bg-gray-100 text-gray-700 hover:bg-blue-100 hover:text-blue-500'}`}
                                                    ...
                                                    {week.map((day, dayIndex) => {
                                                        ...
                                                        return (
                                                            <td
                                                                ...
                                                                className={`border-b border-r border-gray-200 relative transition-all duration-300 hover:bg-blue-50 hover:shadow ${attendance?.status === 'present' ? 'bg-green-50' : attendance?.status === 'absent' ? 'bg-red-50' : 'bg-white'}`}
                                                                style={{ width: '14.2857%' }}
                                                            >
